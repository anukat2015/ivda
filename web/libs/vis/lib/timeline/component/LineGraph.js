...
LineGraph.prototype._drawBarGraphs = function (groupIds, processedGroupData) {
    ...
    for (i = 0; i < combinedData.length; i++) {
        ...
        DOMutil.drawBar(start, posY, width, group.zeroPosition - combinedData[i].y, className, this.svgElements, this.svg, end, title);
        // draw points
        if (group.options.drawPoints.enabled == true) {
            if (end != undefined) {
                start = start + (end - start) * 0.5; // center
            }
            DOMutil.drawPoint(start, posY, group, this.svgElements, this.svg);
        }
    }
};
